...
import { faStar, faAngleRight, faStarHalfStroke } from '@fortawesome/free-solid-svg-icons';
...
function MovieCard() {
    ...
    useEffect(() => { 
        async function fetchData() {
            const response = fetch(`http://localhost:3001/randommovie`).then(res => {
                if(res.ok) {
                    return res.json();
                }
    ...
    return(
        ...
        <h1 id="h1-wt">Watch Today <FontAwesomeIcon icon={faAngleRight} /></h1>
        <div class="row">
            {movie.map(movie =>
            <div class="col-sm-3">
